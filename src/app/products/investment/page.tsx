...
function drawPhone(ctx: CanvasRenderingContext2D, w: number, h: number, s: S, t: number) {
  ...
  ctx.fillStyle = 'rgba(8,14,24,0.97)'; ctx.strokeStyle = 'rgba(59,130,246,0.28)'; ctx.lineWidth = 1.2; ctx.fill(); ctx.stroke();
  const gl = ctx.createRadialGradient(w / 2, h / 2, 0, w / 2, h / 2, pw);
  gl.addColorStop(0, `rgba(30,80,200,${0.08 + 0.04 * Math.sin(t * 1.4)})`); gl.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = gl; ctx.fillRect(0, 0, w, h);
  ...
  ctx.fillStyle = `rgba(59,130,246,${s.notifA as number})`; ctx.shadowBlur = 15 * (s.notifA as number); ctx.shadowColor = 'rgba(59,130,246,0.8)'; ctx.fill(); ctx.shadowBlur = 0;
  ...
  for (const m of msgs) { ctx.beginPath(); ctx.roundRect(m.x, m.y, m.w, 18, 9); ctx.fillStyle = m.sent ? `rgba(29,78,216,${m.a})` : `rgba(20,40,70,${m.a})`; ctx.fill(); }
  ...
  for (let i = pulses.length - 1; i >= 0; i--) { pulses[i].r += 1.2; pulses[i].a -= 0.009; if (pulses[i].a <= 0) { pulses.splice(i, 1); continue; } ctx.beginPath(); ctx.arc(w / 2, py + ph * 0.19, pulses[i].r, 0, Math.PI * 2); ctx.strokeStyle = `rgba(59,130,246,${pulses[i].a})`; ctx.lineWidth = 1; ctx.stroke(); }
}
...
const ACCENT = 'linear-gradient(90deg,#60a5fa,#3b82f6)';
const ACCENT_COLOR = '#60a5fa';
const ACCENT_GRAD = 'linear-gradient(90deg,#1d4ed8,#3b82f6,#60a5fa)';
const BG_COLOR = '#060a0e';
const STATS = [
  { target: 90, suffix: '%', label: '재물 성향 적중률', big: true },
  { target: 7300, suffix: '+', label: '누적 분석 건수', big: false },
  { target: 95, suffix: '%', label: '이용자 만족도', big: false },
  { target: 84, suffix: '%', label: '투자 타이밍 정확도', big: false },
];
const CONCERNS = [
  { emoji: '😓', text: '열심히 버는데 돈이 안 모여요. 제 사주에 재복이 없는 건 아닐까요?', delay: 0 },
  { emoji: '🤷', text: '주식이 맞는지 부동산이 맞는지 모르겠어요. 제 팔자엔 어떤 재테크가 맞을까요?', delay: 140 },
  { emoji: '💭', text: '투자하려는데 지금이 좋은 타이밍인지, 아니면 더 기다려야 할지 너무 고민돼요.', delay: 280 },
  { emoji: '🌊', text: '올해 재물운이 어떤지 미리 알고 싶어요. 큰 결정이 있어서요...', delay: 420 },
];
const REVIEWS = [
  { emoji: '🌟', name: '김○○', age: '40대 남성', rating: 5, text: '부동산 매입 타이밍을 딱 알려줬어요. 그 시기에 매입했더니 1년 만에 많이 올랐어요. 신기할 정도였습니다.' },
  { emoji: '⭐', name: '이○○', age: '30대 여성', rating: 5, text: '주식을 해야 할지 적금을 해야 할지 고민이었는데, 제 사주에 맞는 재테크 방향을 알려줘서 확신이 생겼어요.' },
  { emoji: '✨', name: '박○○', age: '50대 남성', rating: 5, text: '올해 상반기 재물운이 좋다고 했는데 진짜 사업에서 큰 계약이 성사됐어요. 놀라웠습니다.' },
  { emoji: '💫', name: '최○○', age: '30대 남성', rating: 4, text: '재테크 성향 분석이 너무 정확했어요. 제가 모험형인지 안정형인지 사주로 이렇게 구체적으로 알 수 있다니.' },
];
...
function StatItem({ target, suffix, label, big }: { target: number; suffix: string; label: string; big: boolean }) {
  ...
  if (big) return (
    <div className="col-span-2 rounded-2xl p-6 text-center relative overflow-hidden" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(59,130,246,0.22)' }}>
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 70% 55% at 50% 110%,rgba(59,130,246,0.15),transparent)' }} />
...
function BottomCTA() {
  ...
  return (
    <>
      <section className="relative overflow-hidden" style={{ background: '#02060a' }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 110% 50% at 50% 100%, rgba(29,78,216,0.22), transparent)' }} />
        <div className="relative z-10 px-6 pt-14 pb-10 max-w-md mx-auto">
          <div className="text-center mb-10">
            <span className="inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-xs font-bold" style={{ background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.35)', color: '#93c5fd' }}>🔥 재테크 성향 분석 한정 할인</span>
          </div>
          <div className="text-center mb-10">
            <p className="font-black leading-[0.95]" style={{ fontSize: 'clamp(3.6rem,16vw,6rem)', letterSpacing: '-0.03em', background: 'linear-gradient(180deg,#fff 10%,rgba(255,255,255,0.5) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>재테크</p>
            <p className="font-black leading-[0.95]" style={{ fontSize: 'clamp(3.6rem,16vw,6rem)', letterSpacing: '-0.03em', background: 'linear-gradient(180deg,rgba(255,255,255,0.55) 0%,rgba(255,255,255,0.18) 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>성향 분석</p>
          </div>
          <div className="mb-10 space-y-3 max-w-xs mx-auto">
            {[
              { text: '나의 재물 성향 (안정형 vs 공격형)', dim: false },
              { text: '올해 재물운 흐름 및 주요 시기', dim: false },
              { text: '나에게 맞는 재테크 유형 분석', dim: false },
              { text: '투자 행운이 오는 달 vs 조심할 달', dim: false },
              { text: '10년 재물 대운 흐름까지..', dim: true },
            ].map((f, i) => (
          ...
          <div className="text-center mb-8">
            ...
            <div className="flex items-center justify-center gap-3 mb-3">
              ...
              <span className="rounded-full px-3 py-1 text-xs font-bold" style={{ background: 'rgba(59,130,246,0.15)', color: '#93c5fd', border: '1px solid rgba(59,130,246,0.4)' }}>한정 할인</span>
            </div>
            <div className="inline-flex items-center gap-1.5 rounded-full px-5 py-2 text-sm font-black" style={{ background: 'linear-gradient(90deg,#1d4ed8,#3b82f6)', color: 'white', boxShadow: '0 6px 28px rgba(59,130,246,0.4)' }}>지금 결제 시 50% 할인!</div>
          </div>
          <div className="text-center mb-8">
            ...
            <div className="inline-flex items-center rounded-2xl px-10 py-4" style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.07)' }}>
              <span className="tabular-nums font-black text-white" style={{ fontSize: 'clamp(2.6rem,11vw,4rem)', lineHeight: 1, letterSpacing: '-0.03em', textShadow: '0 0 28px rgba(59,130,246,0.5)' }}>{m}</span>
              <span className="font-black mx-1.5" style={{ color: 'rgba(255,255,255,0.25)', fontSize: 'clamp(2rem,8vw,3rem)', lineHeight: 1 }}>:</span>
              <span className="tabular-nums font-black text-white" style={{ fontSize: 'clamp(2.6rem,11vw,4rem)', lineHeight: 1, letterSpacing: '-0.03em', textShadow: '0 0 28px rgba(59,130,246,0.5)' }}>{s}</span>
            </div>
            <p className="text-[10px] mt-3" style={{ color: 'rgba(255,255,255,0.2)' }}>할인 종료 후 자동 갱신</p>
          </div>
          <Link href="/start?cat=investment" className="w-full h-16 rounded-full flex items-center justify-center text-white font-black text-[16px] transition-all hover:scale-[1.015] active:scale-[0.97]" style={{ background: ACCENT_GRAD, boxShadow: '0 0 70px rgba(59,130,246,0.45), 0 10px 40px rgba(0,0,0,0.5)' }}>
            재테크 성향 확인하기 →
          </Link>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50" style={{ background: 'rgba(2,6,10,0.97)', borderTop: '1px solid rgba(59,130,246,0.18)', backdropFilter: 'blur(20px)' }}>
        <div className="max-w-md mx-auto px-4 py-2.5 flex items-center gap-3">
          <div className="flex-shrink-0">
            ...
            <p className="font-black tabular-nums leading-none" style={{ fontSize: '1.55rem', letterSpacing: '-0.03em', color: ACCENT_COLOR, textShadow: '0 0 18px rgba(59,130,246,0.7)' }}>
              {m}<span style={{ color: 'rgba(255,255,255,0.22)', fontSize: '1.1rem', margin: '0 1px' }}>:</span>{s}
            </p>
          </div>
          <Link href="/start?cat=investment" className="flex-1 h-12 rounded-full flex items-center justify-center text-white font-bold text-sm transition-all active:scale-[0.97]" style={{ background: 'linear-gradient(90deg,#1d4ed8,#3b82f6)', boxShadow: '0 0 24px rgba(59,130,246,0.45)' }}>
            재테크 운세 신청하기 →
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ background: BG_COLOR, color: '#fff', overflowX: 'hidden' }}>
      ...
      <div className="relative overflow-hidden" style={{ height: '60vw', maxHeight: '480px', minHeight: '300px' }}>
        ...
        <img src="https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=1200&q=85&fit=crop" alt="재테크성향" className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0" style={{ background: `linear-gradient(to top,${BG_COLOR} 0%,rgba(6,10,14,0.55) 45%,rgba(6,10,14,0.15) 100%)` }} />
        <div className="absolute inset-0" style={{ background: 'linear-gradient(to right,rgba(29,78,216,0.45),transparent 65%)' }} />
        <div className="absolute bottom-0 left-0 w-full px-5 pb-7">
          <div className="flex items-center gap-2 mb-2.5">
            <span className="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold text-white" style={{ background: 'linear-gradient(90deg,#1d4ed8,#3b82f6)' }}>✦ 재테크 성향 분석</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-bold text-white leading-tight drop-shadow-lg">사주로 알아보는<br />재테크 성향</h1>
          <p className="text-sm mt-2" style={{ color: 'rgba(255,255,255,0.52)' }}>재물운 · 투자 성향 · 타이밍 완전 분석</p>
          <div className="flex items-center gap-2 mt-3">
            ...
            <span className="text-xs" style={{ color: 'rgba(255,255,255,0.45)' }}>4.8 · 7,300+ 분석 완료</span>
          </div>
        </div>
      </div>
      ...
      <section className="px-5 pt-8 pb-4 max-w-md mx-auto">
        ...
        <FadeIn delay={100} className="mt-3">
          <div className="rounded-2xl px-5 py-4 flex items-start gap-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div>
              <p className="text-sm text-white leading-relaxed">&ldquo;부동산 매입 타이밍을 딱 알려줬어요. <strong>그 시기에 매입했더니 1년 만에 많이 올랐어요.</strong>&rdquo;</p>
              <p className="text-[11px] mt-1" style={{ color: 'rgba(255,255,255,0.28)' }}>— 김○○, 40대 남성</p>
            </div>
          </div>
        </FadeIn>
      </section>
      ...
      <section className="px-6 py-14 max-w-md mx-auto text-center">
        <FadeIn>
          <p className="font-black leading-[1.1]" style={{ fontSize: 'clamp(1.8rem,7vw,2.8rem)', letterSpacing: '-0.01em' }}>
            <span style={{ color: 'rgba(255,255,255,0.18)' }}>내 팔자에</span>{' '}
            <span style={{ color: 'rgba(255,255,255,0.85)' }}>맞는 재테크가</span>
            <br />
            <span style={{ background: ACCENT, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>따로 있어요.</span>
          </p>
        </FadeIn>
      </section>
      ...
      <div className="relative w-full overflow-hidden" style={{ height: 'clamp(240px, 55vw, 340px)' }}>
        <PhoneCanvas />
        <div className="absolute inset-x-0 top-0 pointer-events-none" style={{ height: '40%', background: `linear-gradient(to bottom,${BG_COLOR} 0%,rgba(6,10,14,0.5) 60%,transparent 100%)` }} />
        <div className="absolute inset-x-0 bottom-0 pointer-events-none" style={{ height: '50%', background: `linear-gradient(to top,${BG_COLOR} 0%,rgba(6,10,14,0.6) 55%,transparent 100%)` }} />
        <div className="absolute bottom-8 left-0 right-0 px-6 text-center">
          <FadeIn>
            <p className="font-black leading-[1.1] mb-1" style={{ fontSize: 'clamp(1.5rem,6vw,2.2rem)', letterSpacing: '-0.01em' }}>
              <span style={{ color: 'rgba(255,255,255,0.5)' }}>내 재물운이 </span>
              <span style={{ background: ACCENT, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>어떻게 되나요?</span>
            </p>
        ...
        <div className="space-y-3">
          ...
          <FadeIn delay={580}>
            <div className="flex items-start gap-3 flex-row-reverse">
              <div className="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center text-sm font-black" style={{ background: 'linear-gradient(135deg,#1d4ed8,#3b82f6)', color: 'white' }}>命</div>
              <div className="rounded-2xl rounded-tr-md px-4 py-3" style={{ background: 'rgba(29,78,216,0.18)', border: '1px solid rgba(59,130,246,0.25)', maxWidth: 'calc(100% - 52px)' }}>
                <p className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.82)' }}>팔자에 맞지 않는 재테크는 새는 독에 물 붓기예요. 당신의 재물 성향이 안정형인지 공격형인지, 어떤 투자 방식이 사주와 맞는지, 올해 재물운의 흐름까지 정밀하게 분석해 드릴게요. 💰</p>
              </div>
            </div>
          </FadeIn>
        </div>
      ...
      <section className="px-6 py-16 max-w-md mx-auto text-center">
        <FadeIn>
          <p className="font-black leading-[1.08]" style={{ fontSize: 'clamp(1.9rem,8vw,3rem)', letterSpacing: '-0.02em' }}>
            <span style={{ color: 'rgba(255,255,255,0.2)' }}>팔자에 맞지 않는 재테크는</span><br />
            <span style={{ background: ACCENT, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>새는 독에</span><br />
            <span style={{ color: 'rgba(255,255,255,0.75)' }}>물 붓기예요.</span>
          </p>
          <p className="text-sm mt-5" style={{ color: 'rgba(255,255,255,0.3)' }}>내 팔자에 맞는 재테크를 알면, 같은 돈도 더 불어납니다.</p>
        </FadeIn>
      </section>
      ...
      <section className="px-5 pb-16 max-w-md mx-auto">
        ...
        <div className="space-y-2">
          {[['💰', '나의 재물 성향 (안정형 vs 공격형 분석)'], ['📊', '주식·부동산·사업 중 내 팔자에 맞는 것'], ['📅', '올해 재물운 흐름 및 행운 시기'], ['⚠️', '손재수 조심 달 & 투자 금지 시기'], ['🔮', '10년 재물 대운 흐름']].map(([icon, text], i) => (
      ...
      <section className="px-5 pb-16 max-w-md mx-auto">
        ...
        <FadeIn>
          <div className="rounded-2xl overflow-hidden relative" style={{ border: '1px solid rgba(255,255,255,0.07)' }}>
            <div className="p-5 space-y-2.5" style={{ background: 'rgba(6,10,20,0.9)' }}>
              {['재물 성향 – 안정·성장형, 꾸준한 복리 투자 적합', '2월·6월 – 재물운 상승, 저축 및 장기 투자 시작 유리', '4월 – ⚠️ 손재수 주의, 충동적 투자 금지', '하반기 재물운 + 10년 대운 흐름 (분석 후 공개)'].map((t, i) => (
                <div key={i} className="rounded-xl px-4 py-3 text-sm" style={{ background: 'rgba(8,18,40,0.7)', border: '1px solid rgba(30,80,200,0.15)', color: 'rgba(255,255,255,0.7)', filter: i > 1 ? 'blur(5px)' : 'none', userSelect: i > 1 ? 'none' : 'auto' }}>{t}</div>
              ))}
            </div>
            <div className="absolute bottom-0 left-0 right-0 h-28 flex items-end justify-center pb-5" style={{ background: `linear-gradient(to top,${BG_COLOR} 55%,transparent)` }}>
              <Link href="/start?cat=investment" className="inline-flex items-center gap-2 rounded-full px-6 py-2.5 text-sm font-bold text-white" style={{ background: 'linear-gradient(90deg,#1d4ed8,#3b82f6)', boxShadow: '0 0 24px rgba(59,130,246,0.4)' }}>🔒 전체 결과 확인하기</Link>
            </div>
          </div>
        </FadeIn>
      </section>
            ...
            <FadeIn key={i} delay={i * 65}>
              <div className="rounded-2xl p-5" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-lg flex-shrink-0" style={{ background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.22)' }}>{r.emoji}</div>
