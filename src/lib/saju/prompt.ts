...
각 섹션은 반드시 2500자 이상 작성한다. 절대로 짧게 쓰지 않는다. 각 항목마다 최소 3~5문장 이상 상세히 서술한다.
이름은 첫 번째 섹션 시작 부분에 딱 한 번만 자연스럽게 언급하고, 이후에는 절대 사용하지 않는다.

## 나의 사주 핵심 성향 (2000자 이상 필수)

작성 기준 (모두 빠짐없이 작성할 것):
- 일간(日干)을 중심으로 성향을 분석한다. 일간의 음양오행적 특성을 상세히 설명한다.
- 격국과 용신을 명시하고 어떤 의미인지 풀어쓴다. 격국이 삶의 방향성에 미치는 영향을 구체적으로 설명한다.
- 신강/신약 여부와 그것이 삶에 어떻게 나타나는지 행동으로 묘사한다.
- 십성 배치에서 드러나는 대인관계 패턴을 구체적으로 쓴다. 비겁·식상·재성·관성·인성 각각의 배치 의미를 설명한다.
- 강점 3가지, 보완점 2가지를 각각 명리학적 근거와 함께 단정문으로 쓴다.
- 합충형해파가 있으면 그것이 성격에 미치는 영향을 상세히 설명한다.
- 이 사주를 가진 사람이 인생에서 반복적으로 겪는 패턴을 설명한다.

## ${input.currentYear}년 병오년 총운 (2000자 이상 필수)

작성 기준 (모두 빠짐없이 작성할 것):
- 첫 문장: "${input.currentYear}년 당신의 한 해는 [한 줄 결론]입니다." 형식으로 시작한다.
- 세운(병오년) 천간 병화(丙火)와 지지 오화(午火)가 일간과 어떤 관계인지 명리학적으로 상세히 설명한다.
- 대운과 세운이 겹치는 지점에서 발생하는 변화를 구체적으로 쓴다.
- 상반기(1~6월)와 하반기(7~12월)를 나눠서 각 200자 이상으로 흐름을 설명한다.
- 재물운 흐름을 명리학적 근거와 함께 상세히 설명한다.
- 직업/커리어 흐름을 명리학적 근거와 함께 상세히 설명한다.
- 관계운 흐름을 명리학적 근거와 함께 상세히 설명한다.
...
각 월마다 반드시 1000자 이상 작성한다. 절대로 짧게 쓰지 않는다. 각 소항목(직업/재물/관계/건강)마다 최소 3문장 이상 서술한다.
이름은 절대 사용하지 않는다. 6개월 모두 빠짐없이 작성해야 하며, 한 달도 생략하거나 축약하지 않는다.
