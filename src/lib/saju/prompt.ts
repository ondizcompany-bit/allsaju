...
const BASIC_INFO = (input: PromptInput) => {
  const isPartner = input.catId === 'reunion' || input.catId === 'secret';
...
- ⚠️ 5개 섹션 모두 반드시 완성한다. 절대 중간에 끊기지 않는다.`;
  ...
  // 재회·속궁합 전용 자미두수 섹션 (부처궁·인연 중심)
  if (isPartner) {
    const topicLabel = input.catId === 'reunion' ? '재회 인연' : '궁합';
    return {
      system: SYSTEM_BASE,
      user: `${infoBlock}
