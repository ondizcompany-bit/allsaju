...
- ⚠️ 나이는 절대 직접 계산하지 않는다. 반드시 [분석 대상]에 적힌 "현재 나이: 만 OO세"를 그대로 사용한다. 생년월일에서 나이를 계산하면 오류가 발생하므로 금지한다.
- 각 소제목 단락은 반드시 200자 이상의 긴 문단 2~3개로 작성한다.
- 마크다운 헤딩(##)으로 소제목을 구분하고, 소제목은 감성적이고 시적인 한 줄 문장으로 만든다.
- 한국어로 작성한다.
- ⚠️ 절대 중간에 끊기지 않는다. 요청한 모든 섹션(##)을 반드시 마지막 문장까지 완전히 작성하고 마무리한다. 공간이 부족하면 각 문단을 조금 줄이더라도 모든 섹션을 완성해야 한다. 미완성 결과물은 절대 허용되지 않는다.`;
