...
const BASIC_INFO = (input: PromptInput) => {
  const isPartner = input.catId === 'reunion' || input.catId === 'secret' || input.catId === 'pregnancy-date' || input.catId === 'baby-dna';
  ...
  const emotionNote = input.catId === 'reunion'
...
- ⚠️ 5개 섹션 모두 반드시 완성한다. 절대 중간에 끊기지 않는다.`;
  ...
  if (isPartner) {
    const topicLabel = input.catId === 'reunion' ? '재회 인연'
