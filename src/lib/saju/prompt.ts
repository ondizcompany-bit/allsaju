...
상반기와 하반기 중 ${topicLabel}에 더 유리한 시기를 구체적으로 짚고, ${input.name}님이 ${input.currentYear}년 인연 흐름을 가장 잘 타기 위한 핵심 키워드 3가지로 마무리한다.`,
  ...
  // "타고난 매력" 전용 자미두수 섹션 — 매력을 자미두수 고유 궁(명궁·천이궁·복덕궁·부처궁)으로 재해석.
  // 사주 챕터에서 이미 다룬 스타일링·시선·스킨십·어필 팁과 겹치지 않게, 별의 조합이 만드는 "이미지·분위기·인상" 위주로 서술한다.
  if (input.catId === 'charm') {
    return {
      system: SYSTEM_BASE,
      user: `${infoBlock}
...
아래는 자미두수 심층 분석 섹션 5개다. UI 배너 포함 정확히 아래 순서대로 작성한다.
⚠️ 이 상품은 "타고난 매력"이 주제다. 모든 섹션을 매력과 연결해 서술하되, 스타일링·옷차림·시선 처리·스킨십처럼 이미 사주 분석에서 다룬 실전 팁은 반복하지 않는다. 대신 자미두수 고유의 궁(宮)과 별이 만들어내는 "이미지·분위기·인상·기운"을 중심으로, 사주와는 다른 각도에서 매력을 풀어낸다.

## ✦✦✦ 자미두수 심층 분석 ✦✦✦

위 줄은 UI 배너다. 내용 없이 그대로 두고 바로 다음 섹션을 작성한다.

## 🌌 명궁으로 보는 매력의 원형 — [${input.name}님만의 감성적 한 줄 소제목]

명궁(命宮)은 자미두수에서 타고난 본성과 첫인상의 핵심 궁이다. ${input.name}님의 명궁 주성(主星)이 만들어내는 고유한 분위기와 이미지가 무엇인지, 사람들이 ${input.name}님을 처음 봤을 때 무의식적으로 느끼는 기운이 어떤 것인지 서술한다.

보좌성·흉성 조합이 이 이미지에 어떤 색깔을 더하는지 설명하고, "이 명궁을 가진 사람은 OO한 인상으로 기억된다"는 핵심 문장으로 마무리한다.

## 🌊 천이궁으로 보는 바깥에서의 매력 — [${input.name}님만의 감성적 한 줄 소제목]

천이궁(遷移宮)은 자미두수에서 외부 활동·낯선 자리·타지에서의 에너지를 담은 궁이다. ${input.name}님이 집·익숙한 곳을 벗어나 새로운 사람들 사이에 있을 때 유독 매력이 살아나는 이유를 천이궁 성계로 분석한다.

낯선 모임, 여행지, 처음 만나는 자리에서 ${input.name}님의 매력이 어떻게 다르게 드러나는지 구체적으로 짚어주며 마무리한다.

## 🌸 복덕궁으로 보는 내면의 기품 — [${input.name}님만의 감성적 한 줄 소제목]

복덕궁(福德宮)은 타고난 정신적 여유와 내면의 풍요를 담당하는 궁이다. ${input.name}님의 복덕궁 성계가 만들어내는 여유·기품·분위기가 어떻게 은은한 매력으로 이어지는지 서술한다.

이 내면의 기품이 겉으로 드러날 때 사람들이 ${input.name}님에게서 느끼는 "말로 설명하기 힘든 끌림"이 무엇인지 감성적으로 풀어 쓰며 마무리한다.

## 💕 부처궁으로 보는 관계 속 매력 — [${input.name}님만의 감성적 한 줄 소제목]

부처궁(夫妻宮)은 연애와 관계 속에서 드러나는 에너지를 담은 궁이다. ${input.name}님이 썸이나 연애 관계 안에 있을 때 평소와 다르게 발산되는 매력 포인트를 부처궁 성계로 분석한다.

가까워질수록 더 매력적으로 느껴지는 ${input.name}님만의 관계 속 매력이 무엇인지 짚어주며 마무리한다.

## 🔭 ${input.currentYear}년 자미두수 매력·인기운 총평 — [${input.name}님만의 감성적 한 줄 소제목]

자미두수 세운(歲運) 분석으로 ${input.currentYear}년 ${input.name}님의 명궁·천이궁·복덕궁에 어떤 기운이 들어오는지 종합해 올해 매력·인기운의 큰 흐름을 서술한다.

상반기와 하반기 중 사람들의 시선과 관심을 유독 많이 받게 되는 시기를 구체적으로 짚고, ${input.currentYear}년 ${input.name}님의 매력을 요약하는 핵심 키워드 3가지로 마무리한다.`,
    };
  }
...
상반기·하반기 중 ${topicLabel}에 더 유리한 시기를 짚고, 올해 가장 중요한 달을 구체적으로 언급하며 ${input.currentYear}년 인연 핵심 키워드 3가지로 강렬하게 마무리한다.`,
  ...
  // "타고난 매력" 전용 자미두수 6궁 분석 — 매력을 자미두수 고유 궁으로 재해석.
  // 사주 챕터의 스타일링·시선·스킨십·어필 팁과 겹치지 않게, 별의 조합이 만드는 "이미지·분위기·인상" 위주로 서술한다.
  if (input.catId === 'charm') {
    return {
      system: SYSTEM_BASE,
      user: `${infoBlock}
...
명궁(命宮)은 자미두수에서 타고난 본성과 첫인상의 핵심 궁이다. ${input.name}님의 명궁 주성(主星)이 만들어내는 고유한 분위기와 이미지가 무엇인지, 사람들이 ${input.name}님을 처음 봤을 때 무의식적으로 느끼는 기운이 어떤 것인지 깊이 있게 서술한다.

보좌성·흉성 조합이 이 이미지에 어떤 색깔을 더하는지 설명하고, "이 명궁을 가진 사람은 OO한 인상으로 기억된다"는 핵심 문장으로 마무리한다.

## 🌊 천이궁으로 보는 바깥에서의 매력 — [${input.name}님만의 감성적 한 줄 소제목]

천이궁(遷移宮)은 자미두수에서 외부 활동·낯선 자리·타지에서의 에너지를 담은 궁이다. ${input.name}님이 익숙한 곳을 벗어나 새로운 사람들 사이에 있을 때 유독 매력이 살아나는 이유를 천이궁 성계로 분석한다.
...
## 👥 노복궁으로 보는 사교 속 매력 — [${input.name}님만의 감성적 한 줄 소제목]

노복궁(奴僕宮)은 자미두수에서 친구·동료·인맥 관계의 에너지를 담은 궁이다. ${input.name}님이 여러 사람이 모인 자리에서 발휘하는 사교적 매력과, 사람들 사이에서 자연스럽게 중심이 되거나 신뢰를 얻는 방식을 노복궁 성계로 분석한다.

모임·회식·단체 활동에서 ${input.name}님의 매력이 어떻게 발휘되는지 구체적으로 짚어주며 마무리한다.

## 🔭 ${input.currentYear}년 자미두수 매력·인기운 총평 — [${input.name}님만의 감성적 한 줄 소제목]

자미두수 세운(歲運) 분석으로 ${input.currentYear}년 ${input.name}님의 명궁·천이궁·복덕궁·노복궁에 어떤 기운이 들어오는지 종합해 올해 매력·인기운의 큰 흐름을 깊이 풀어 쓴다.

상반기와 하반기 중 사람들의 시선과 관심을 유독 많이 받게 되는 시기를 구체적으로 짚고, ${input.currentYear}년 ${input.name}님의 매력을 요약하는 핵심 키워드 3가지로 강렬하게 마무리한다.`,
    };
  }
